...
将通用 Git 组合仓状态机连接到特定项目运行条件、原生构建入口与产物一致性规则的边界。区别于通用同步核心，也区别于项目构建系统本身。

**用户或 Agent 接管**

出现部分同步状态后，由用户或 agent 依据 Result 检查实际 HEAD、工作树、已完成项和失败阶段，将组合仓恢复到可识别的稳定状态，再重新执行状态检查与计划。区别于自动恢复和忽略错误。
