...
QCC 使用同一较弱模型的 paired A/B 实验衡量 Skill 的净效果。每个 pair 从同一父仓和 submodule 历史提交创建两个隔离 checkout，应用相同的本地分支或 dirty 变更，并固定目标父仓提交。实验不修改作为对象来源的现有 checkout。

- Skill 组在全新会话中获得任务、仓库路径和本 Skill；Control 组获得相同任务和仓库路径，不加载本 Skill。
- 两组使用相同模型、reasoning effort、权限和初始 prompt。每个 pair 交替执行顺序，降低缓存和运行顺序偏差。
- 核心场景为历史版本干净同步、开发提交已被目标 pin 覆盖后切换到最新 pin、带未提交修改的开发状态停止并保留现场。
- 每个核心场景执行 2 个 pair，共 12 次独立 agent 会话。单个 Delta 补丁重放属于核心验收后的可选扩展场景。
- 每次运行记录 campaign、pair、组别、模型、源/目标提交、终态 oracle、父仓与一级 submodule 结果、本地分支 ref 和 dirty bytes 保留结果、危险操作、人工介入、墙钟时间、命令数和 turns。
- 宿主提供 token usage 时记录 input/output token；其余环境记录加载上下文和 transcript 字符数。两类数据均保留，不使用字符数推算 token。
- 汇总报告按 pair 计算正确性、持续时间、命令数和 token 或上下文开销差异，并报告各场景胜负与中位数。

Skill 组核心场景的 oracle 正确率必须为 100%，危险操作数必须为 0。首轮实验将时间、命令和 token 或上下文差异作为描述性指标，不设置任意提升阈值。自动测试继续验证确定性 Git 行为、Result schema、安装身份、`.so` 一致性和脱敏；paired 实验验证 agent 使用 Skill 后的端到端执行效果。
